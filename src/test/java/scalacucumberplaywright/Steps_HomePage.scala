    ...
    sortType match {
      ...
      */
      case "Price (high to low)" =>
        val noSortPricesInFloatWithoutSign = driverMethods.getAllTextFromAListOfElements(selectors.priceList).asScala.toList.map(_.substring(1)).map(_.toFloat)
        ...
        val priceAfterSortByUI = driverMethods.getAllTextFromAListOfElements(selectors.priceList).asScala.toList.map(_.substring(1)).map(_.toFloat)
      ...
      */
      case "Name (Z to A)" =>
        val noSortNames = driverMethods.getAllTextFromAListOfElements(selectors.productList).asScala.toList
        val sortedNamesZtoA = noSortNames.sorted.reverse.asJava
        driverMethods.selectFromDropdownUsingText(selectors.productSort, sortType)
      ...
      case "Name (A to Z)" =>
        val noSortNames = driverMethods.getAllTextFromAListOfElements(selectors.productList).asScala.toList
        val sortedNamesAtoZ = noSortNames.sorted.asJava
        driverMethods.selectFromDropdownUsingText(selectors.productSort, sortType)
        val namesAfterSortByUI = driverMethods.getAllTextFromAListOfElements(selectors.productList)
